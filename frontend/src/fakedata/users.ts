...
export const fakedata_getUser = async (username: string) => {
    const result = await fetch("http://localhost:3000/users.json", {cache: "no-store"});
...
export const fakedata_getUserScore = async (username: string) => {
    const result1 = await fetch("http://localhost:3000/user_achievements.json", {cache: "no-store"});
    ...
    const result2 = await fetch("http://localhost:3000/achievements.json", {cache: "no-store"});
